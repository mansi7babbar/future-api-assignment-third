...
class AverageRetweetsPerTweetSpec extends AsyncFlatSpec with BeforeAndAfterAll with MockitoSugar {
  ...
  val averageRetweetsPerTweet = new AverageRetweetsPerTweet(mockRetrieveTweets)
  "getAverageRetweetsPerTweet" should "give average number of retweets per tweet" in {
    averageRetweetsPerTweet.getAverageRetweetsPerTweet("#sunday").map(averageRetweetsPerTweet => assert(averageRetweetsPerTweet == 2))
  }
}

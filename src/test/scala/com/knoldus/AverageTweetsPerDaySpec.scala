...
class AverageTweetsPerDaySpec extends AsyncFlatSpec with BeforeAndAfterAll with MockitoSugar {
  ...
  val averageTweetsPerDay = new AverageTweetsPerDay(mockRetrieveTweets)
  "getAverageTweetsPerDay" should "give average number of tweets per day" in {
    averageTweetsPerDay.getAverageTweetsPerDay("#sunday").map(averageTweetsPerDay => assert(averageTweetsPerDay == 1))
  }
}
